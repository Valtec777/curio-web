      ...
      <div className="grid-2">
        <section className="panel">
          ...
          <form action={createAgendaEvent} className="form-stack">
            ...
            <div className="form-row">
              ...
              <div className="field">
                <label>Título *</label>
                <input className="input" name="title" placeholder="Ex.: Aula de Matemática" required />
              </div>
            </div>
            ...
            <div className="field">
              <label>Link da aula/reunião</label>
              <input className="input" type="url" name="meetingUrl" placeholder="https://..." />
            </div>
            <div className="field">
              <label>Local / observação de acesso</label>
              <input className="input" name="location" placeholder="Online, Sala 2..." />
            </div>
        ...
        <section className="panel">
          <div className="notice">
            Reuniões com família podem ficar visíveis apenas para o responsável desmarcando “Mostrar para o aluno”. A RLS respeita essas escolhas no banco, não apenas na tela.
          </div>
      ...
      <section className="panel">
        <div className="panel-head">
          <div>
            ...
            <p>Ações de situação preservam o mesmo evento e os vínculos.</p>
          </div>
        </div>
        ...
        {events?.length ? (
          <div className="form-stack">
            {events.map((event: any) => {
              const participant = event.agenda_event_students?.[0]?.students;
              return (
                <article className="mission-card" key={event.id}>
                  ...
                  <div className="flex gap-8 wrap mt-12">
                    ...
                    {event.meeting_url && <a className="button button-secondary button-small" href={event.meeting_url} target="_blank" rel="noreferrer">Entrar na aula ↗</a>}
                  </div>
                  <div className="plan-admin-actions mt-12">
                    {event.status === "scheduled" && (
                      <form action={setAgendaEventStatus}>
                        <input type="hidden" name="eventId" value={event.id} />
                        <input type="hidden" name="status" value="completed" />
                        <button className="button button-secondary button-small" type="submit">Marcar concluído</button>
                      </form>
                    )}
                    {event.status !== "cancelled" && (
                      <form action={setAgendaEventStatus}>
                        ...
                        <button className="button button-ghost button-small" type="submit">Cancelar encontro</button>
                      </form>
                    )}
                  </div>
                </article>
