...
async function copySourceFile({ supabase, userId, draft }: { supabase: any; userId: string; draft: any }) {
  ...
  const { error: uploadError } = await supabase.storage.from("teacher-materials").upload(path, blob, {
    contentType: draft.source_mime_type || blob.type || "application/octet-stream",
    upsert: false,
  });
...
export async function convertPreparationDraft(formData: FormData) {
  const parsed = z.object({
    ...
    outputType: z.enum(["material", "activity", "assessment", "notebook_pdf"]),
  }).safeParse({ draftId: formData.get("draftId"), outputType: formData.get("outputType") });
  ...
  const { data: draft } = await supabase.from("content_preparation_drafts")
    .select("id,title,source_text,source_file_path,source_file_name,source_mime_type,subject_id,grade_id,theme,objective,notes")
    .eq("id", parsed.data.draftId)
    .eq("created_by_teacher_id", teacher.id)
    .maybeSingle();
  if (!draft) back(parsed.data.draftId, "erro", "Rascunho não encontrado.");

  const outputType = parsed.data.outputType === "activity" ? "material" : parsed.data.outputType;
  ...
  if (parsed.data.outputType === "notebook_pdf" && draft.source_file_path && !["application/pdf", "image/png", "image/jpeg", "image/webp"].includes(draft.source_mime_type || "")) {
    back(draft.id, "erro", "Para Caderno/PDF, a fonte atual ainda precisa ser convertida para PDF ou imagem. O processador de documentos fará essa etapa quando estiver habilitado.");
  }
  if (parsed.data.outputType === "notebook_pdf" && !draft.source_file_path) {
    back(draft.id, "erro", "Para criar o Caderno agora, anexe um PDF ou imagem ao rascunho. A geração visual de PDF a partir de texto será conectada ao modelo CURIÓ depois.");
  }
  ...
  if (parsed.data.outputType === "material" || parsed.data.outputType === "activity") {
    const result = await supabase.from("materials").insert({
      ...
      material_type: materialType(draft.source_mime_type),
      file_path: filePath,
      status: "draft",
    });
    error = result.error;
    destination = "/professor/materiais";
  } else if (parsed.data.outputType === "notebook_pdf") {
