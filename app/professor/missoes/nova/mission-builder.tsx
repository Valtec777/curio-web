...
export function MissionBuilder({ idempotencyKey, subjects, grades, skills, characters, students, initialDraft }: {
  ...
  const initialQuestions: QuestionState[] = prepared.length
    ? prepared.map((question, index) => ({ id: index + 1, type: question.type, initial: question }))
  ...
  return <form action={createMissionWithQuestions} className="form-stack">
    ...
    <section className="panel">
      <div className="panel-head"><div><h2>Informações da missão</h2><p>Defina o contexto uma vez; depois revise as questões.</p></div></div>
      {initialDraft ? <div className="notice mb-16"><strong>Rascunho carregado.</strong> Revise todos os campos antes de criar a Missão. Nada foi publicado automaticamente.</div> : null}
    ...
    <section className="panel">
      <div className="panel-head"><div><h2>Questões</h2><p>Você pode misturar múltipla escolha, verdadeiro ou falso e discursivas na mesma Missão Cuca.</p></div></div>
      ...
      <div className="mission-builder-list mt-12">{questions.map((question, index) => {
        const initial = question.initial;
        const optionValues = initial?.options || [];
        return <article className="mission-builder-question" key={question.id}>
          ...
          <div className="field"><label>Pista <span className="field-optional">opcional</span></label><input className="input" name={`q${index}Hint`} defaultValue={initial?.hint || ""} /></div>
        </article>;
      })}</div>
      <div className="flex gap-8 wrap mt-16"><button className="button button-secondary" type="button" onClick={addQuestion} disabled={questions.length >= 20}>+ Adicionar outra questão</button><button className="button button-primary" type="submit">Criar missão</button></div>
    </section>
  </form>;
}
