...
function questionFromForm(formData: FormData, index: number): MissionQuestionInput | null {
  ...
  if (type === "open_text") {
    return { position: index + 1, type, prompt, hint, options: [], correctAnswer: "" };
  }
...
export async function createMissionWithQuestions(formData: FormData) {
  ...
  if (!parsed.success) {
    redirect(`/professor/missoes/nova?erro=${encodeURIComponent("Revise os dados gerais da missão.")}`);
  }

  const questions = Array.from({ length: parsed.data.questionCount }, (_, index) => questionFromForm(formData, index));
  if (questions.some((question) => !question)) {
    redirect(`/professor/missoes/nova?erro=${encodeURIComponent("Revise todas as questões. Múltipla escolha precisa de 4 alternativas e uma resposta correta.")}`);
  }
  ...
  if (metadataError) {
    redirect(`/professor/missoes/nova?erro=${encodeURIComponent("A missão foi criada, mas os detalhes adicionais não puderam ser salvos.")}`);
  }
  ...
  const { data: savedQuestions, error: questionsError } = await supabase
    ...
    .upsert(questionRows, { onConflict: "mission_id,position" })
    .select("id,position");

  if (questionsError || (savedQuestions ?? []).length !== normalizedQuestions.length) {
    redirect(`/professor/missoes/nova?erro=${encodeURIComponent("A missão foi salva como rascunho, mas nem todas as questões puderam ser registradas.")}`);
  }
  ...
  for (const question of normalizedQuestions) {
    ...
    if (question.type === "open_text") {
      ...
      const { error: keyError } = await supabase.from("mission_question_answer_keys").upsert({
        question_id: questionId,
        correct_value: question.correctAnswer,
      }, { onConflict: "question_id" });
      if (keyError) {
        redirect(`/professor/missoes/nova?erro=${encodeURIComponent("As questões foram salvas, mas um gabarito não pôde ser registrado.")}`);
      }
    }
  }
  ...
  redirect(`/professor/missoes?sucesso=${encodeURIComponent(studentIds.length ? `Missão com ${normalizedQuestions.length} questão(ões) publicada para ${studentIds.length} aluno(s).` : `Missão com ${normalizedQuestions.length} questão(ões) salva como rascunho.`)}`);
}
