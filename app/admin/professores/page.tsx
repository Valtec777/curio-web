...
import { moveTeacherToTrash } from "./actions";
...
export default async function AdminTeachersPage({
  ...
  const [{ data: teachers }, { data: invitations }, { data: trashItems }] = await Promise.all([
    supabase
      .from("teachers")
    ...
    supabase
      ...
      .select("id,email,full_name,status,sent_at,accepted_at,last_error")
      .eq("role", "teacher")
      .is("deleted_at", null)
      .order("created_at", { ascending: false })
      .limit(50),
  ...
  return (
    <>
      <PageHeader
        ...
        title="Professores"
        description="Cadastre, edite, desative, exclua e restaure professores sem apagar o histórico pedagógico."
      />
      ...
      <div className="grid-2">
        <section className="panel">
          <div className="panel-head">
            <div>
              ...
              <p>O professor recebe um link para definir a própria senha.</p>
            </div>
          </div>
        ...
        <section className="panel">
          <div className="panel-head"><div><h2>Convites recentes</h2></div></div>
          {invitations?.length ? (
              ...
              {invitations.map((invite: any) => (
                <article className="access-invite-card" key={invite.id}>
                  <div className="flex space-between gap-8 wrap">
                    ...
                    <Badge tone={invite.status === "accepted" ? "green" : invite.status === "error" ? "pink" : "yellow"}>{invite.status}</Badge>
                  </div>
      ...
      <section className="panel">
        <div className="panel-head">
          <div>
            ...
            <p>{visibleTeachers.length} professor(es) fora da Lixeira.</p>
          </div>
        </div>
              ...
              <article className="person-admin-card" key={teacher.id}>
                <div className="flex space-between gap-8 wrap">
                  <div>
                    ...
                    <p>{teacher.profiles?.full_name}</p>
                  </div>
                  <Badge tone={teacher.active ? "green" : "neutral"}>{teacher.active ? "Ativo" : "Acesso retirado"}</Badge>
                </div>
                <p>{teacher.professional_description || "Descrição profissional não preenchida"}</p>
                <small className="muted">{teacher.phone_whatsapp || teacher.profiles?.phone_whatsapp || "Sem telefone"}</small>

                <details className="plan-editor">
                  <summary>Editar professor</summary>
                ...
                <details className="plan-editor">
                  ...
                  <form action={moveTeacherToTrash} className="form-stack compact-form">
                    ...
                    <p className="muted">O acesso de professor será retirado, mas alunos, turmas, missões e correções permanecerão no histórico.</p>
