...
export async function moveDocumentToTrash(formData: FormData) {
  ...
  const parsed = removeDocumentSchema.safeParse({
  ...
  const { data: document, error: documentError } = await supabase
    .from("documents")
    .select("id,title,document_type,file_path,student_id,guardian_id,subscription_id,visible_to_guardian,created_by_user_id,created_at,deleted_at")
    .eq("id", parsed.data.documentId)
    .maybeSingle();

  if (documentError || !document || document.deleted_at) {
    redirect(`/admin/documentos?erro=${encodeURIComponent("Documento não encontrado ou já removido.")}`);
  }
  ...
  const { error: trashError } = await supabase.from("trash_items").insert({
    ...
    entity_snapshot: {
      ...
      visible_to_guardian: document.visible_to_guardian,
  ...
  if (trashError?.code === "23505") {
    redirect(`/admin/documentos?erro=${encodeURIComponent("Este documento já está na Lixeira.")}`);
  }
  if (trashError) {
    redirect(`/admin/documentos?erro=${encodeURIComponent("Não foi possível enviar o documento para a Lixeira.")}`);
  }
  ...
  if (deleteError) {
    await supabase
      ...
      .is("restored_at", null);
    redirect(`/admin/documentos?erro=${encodeURIComponent("O documento não foi removido da operação. Nenhuma entrada incompleta foi mantida na Lixeira.")}`);
  }
