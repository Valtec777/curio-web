...
export default async function ReferralLandingPage({
  ...
  if (!landing?.program_active) {
    return <main className="site-shell section"><div className="flex space-between align-center wrap" style={{ marginBottom: 38 }}><Logo /><Link className="button button-secondary" href="/">Voltar ao início</Link></div><section className="panel" style={{ maxWidth: 720, margin: "70px auto" }}><div className="eyebrow">Convite CURIÓ</div><h1>Este convite não está disponível agora.</h1><p className="muted">A campanha pode ter sido encerrada ou pausada. Você ainda pode conhecer o CURIÓ pela página principal.</p><Link className="button button-primary mt-16" href="/">Conhecer o CURIÓ</Link></section></main>;
  }

  const inviter = landing.owner_type === "teacher" ? `Você recebeu um convite de ${landing.owner_name}.` : "Uma família CURIÓ convidou você para conhecer a plataforma.";
      ...
      <section className="section curio-public-hero">
        <div className="site-shell lead-grid">
          <div className="lead-copy">
            <div className="eyebrow eyebrow-green">Convite CURIÓ</div>
          ...
          <form className="lead-form" action={createEnrollmentRequest} id="quero-conhecer">
            ...
            {lead === "sucesso" && <div className="form-message form-success">Recebemos seu interesse. A equipe CURIÓ entrará em contato com você.</div>}
