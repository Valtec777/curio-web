...
const copy: Record<string, { eyebrow: string; title: string; description: string }> = {
  hoje: { eyebrow: "Explorador Curió", title: "Hoje", description: "Seu ponto de partida para as aventuras de hoje." },
  agenda: { eyebrow: "Explorador Curió", title: "Agenda", description: "Encontros e compromissos que fazem parte do seu caminho." },
  caminho: { eyebrow: "Explorador Curió", title: "Meu Caminho", description: "Veja o que já passou, o que está acontecendo e o que vem depois." },
  perfil: { eyebrow: "Explorador Curió", title: "Meu perfil", description: "Seu espaço no Curió." },
  caderno: { eyebrow: "Caderno Curió", title: "Meu Caderno", description: "Atividades fora da tela para escrever, pensar e mostrar seu raciocínio." },
  conquistas: { eyebrow: "Universo Curió", title: "Conquistas", description: "Cada conquista marca uma atitude importante na sua jornada." },
  descobertas: { eyebrow: "Universo Curió", title: "Descobertas", description: "Curiosidades e conteúdos que você desbloqueou pelo caminho." },
  "modo-pensar": { eyebrow: "Cursos Livres Curió", title: "Modo Pensar", description: "Cursos livres criados pela Administração, com trilhas curtas, progresso e certificado de conclusão." },
...
export default async function StudentSectionPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<{ erro?: string; sucesso?: string }> }) {
  ...
  if (section === "caderno") {
    ...
    return <><PageHeader {...sectionCopy} /><section className="panel family-highlight"><strong>Escrever também é aprender.</strong><p className="mb-0">As atividades do Caderno Curió ficam fora do quiz: abra a folha/PDF, faça no caderno ou impresso, fotografe quando solicitado e envie para a professora revisar.</p></section><section className="panel">{assignments?.length ? <div className="form-stack">{assignments.map((item: any) => <article className="mission-card" key={item.id}><div className="flex space-between gap-8 wrap"><div><Badge tone="blue">{item.notebook_activities?.subjects?.name || "Caderno Curió"}</Badge><h3>{item.notebook_activities?.title}</h3><p>{item.notebook_activities?.description}</p></div><Badge tone={item.status === "completed" ? "green" : item.status === "submitted" ? "yellow" : "neutral"}>{item.status}</Badge></div><div className="flex gap-8 wrap">{item.notebook_activities?.worksheet_path && <a className="button button-secondary button-small" href={item.notebook_activities.worksheet_path} target="_blank" rel="noreferrer">Abrir atividade / PDF ↗</a>}{item.submission_photo_path && <Badge tone="green">Foto enviada</Badge>}{item.stars_awarded > 0 && <Badge tone="yellow">+{item.stars_awarded} ★</Badge>}</div>{item.teacher_note && <div className="notice">Recado da professora: {item.teacher_note}</div>}</article>)}</div> : <EmptyState title="Nenhuma atividade de caderno agora" description="As próximas atividades aparecem aqui quando forem enviadas." />}</section></>;
  }
      ...
      <section className="panel course-library-panel">
        ...
        {courses?.length ? <div className="course-grid">{courses.map((course: any) => {
          ...
          return <article className="course-card" key={course.id}>
            ...
            <h3>{course.title}</h3><p>{course.summary || course.description || "Uma trilha livre do Universo Curió."}</p>
