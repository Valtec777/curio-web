...
export default async function StudentProfilePage({searchParams}:{searchParams:Promise<{erro?:string;sucesso?:string}>}) {
  ...
  return <>
    ...
    <section className="student-profile-hero panel"><div className="flex gap-16 align-center wrap">{selectedAvatar?<Image src={selectedAvatar.assets.avatar||selectedAvatar.assets.principal} alt={`Avatar ${selectedAvatar.name}`} width={120} height={120}/>:<div className="achievement-icon">★</div>}<div><span className="student-kicker">{game?.level_name||"Explorador Curió"}</span><h2>{details?.preferred_name||details?.full_name}</h2><p>{grade?.name||"Ano não informado"}{details?.school_name?` · ${details.school_name}`:""}</p><div className="flex gap-8 wrap"><Badge tone="yellow">★ {game?.stars??0} estrelas</Badge><Badge tone="pink">🔥 {game?.streak_days??0} dias</Badge><Badge tone="purple">🏅 {achievementCount??0} conquistas</Badge></div></div></div></section>
    <section className="panel"><div className="panel-head"><div><h2>Escolha seu avatar</h2><p>Clique em um personagem. A escolha fica salva e passa a aparecer na sua lateral e para a equipe que acompanha você.</p></div>{selectedAvatar&&<Badge tone="green">Atual: {selectedAvatar.name}</Badge>}</div>{avatars.length?<div className="student-avatar-grid">{avatars.map((character:any)=>{const selected=character.id===game?.avatar_character_id;const src=character.assets.avatar||character.assets.principal;return <article className={`student-avatar-card${selected?" is-selected":""}`} key={character.id}><Image src={src} alt={`Avatar ${character.name}`} width={112} height={112}/><h3>{character.name}</h3><form action={chooseStudentAvatar}><input type="hidden" name="characterId" value={character.id}/><AvatarSubmitButton selected={selected}/></form></article>;})}</div>:<p className="muted">Os avatares do CURIÓ ainda não estão disponíveis.</p>}</section>
