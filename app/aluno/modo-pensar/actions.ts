...
export async function startStudentCourse(formData: FormData) {
  ...
  if(error) redirect(`/aluno/modo-pensar?erro=${encodeURIComponent("Não foi possível iniciar este curso.")}`);
...
export async function completeStudentCourseModule(formData: FormData) {
  ...
  if(error) redirect(`/aluno/modo-pensar/${encodeURIComponent(parsed.data.slug)}?erro=${encodeURIComponent("Não foi possível concluir esta etapa.")}`);
