...
export default async function StudentAchievementsPage() {
  ...
  return <>
    ...
    <PageHeader eyebrow="Explorador Plumareli" title="Suas conquistas" description={`Você já desbloqueou ${unlocked} de ${total} selos. Continue explorando: cada selo marca um tipo diferente de avanço.`} />
    ...
    {catalog?.length ? <div className="student-achievement-grid">{catalog.map((item:any)=>{const isEarned=earned.has(item.id);return <article className={`student-achievement-card${isEarned?" is-earned":" is-locked"}`} key={item.id}><div className="student-achievement-medal"><CurioIcon name={iconName(item.icon)} /></div><div><div className="flex gap-8 wrap"><Badge tone={isEarned?"green":"neutral"}>{isEarned?"Desbloqueada":"Ainda bloqueada"}</Badge></div><h3>{item.name}</h3><p>{item.description}</p><small>{isEarned ? "Conquista registrada na sua jornada." : item.unlock_hint || "Continue aprendendo para desbloquear."}</small></div></article>})}</div> : <EmptyState title="Catálogo em preparação" description="As conquistas Plumareli aparecerão aqui." />}
  </>;
}
