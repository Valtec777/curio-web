...
const sectionCopy: Record<string, { eyebrow: string; title: string; description: string }> = {
  ...
  mensagens: { eyebrow: "Ninho da Família", title: "Mensagens", description: "Conversas com a equipe e professores do Plumareli." },
  ...
  suporte: { eyebrow: "Ninho da Família", title: "Suporte Plumareli", description: "Abra uma solicitação e acompanhe dúvidas de conta, plataforma ou financeiro." },
};
...
export default async function FamilySectionPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<{ erro?: string; sucesso?: string }> }) {
  ...
  if (["plano", "contrato", "pagamentos"].includes(section)) {
    ...
    if (section === "plano") return <><PageHeader {...copy} /><section className="panel">{subs?.length ? <div className="grid-3">{subs.map((sub: any) => <article className="mission-card" key={sub.id}><Badge tone={sub.status === "active" ? "green" : "neutral"}>{sub.status}</Badge><h3>{sub.plans?.name || "Plano Plumareli"}</h3><p>{studentName.get(sub.student_id)}</p><strong>R$ {Number(sub.agreed_monthly_price ?? sub.plans?.monthly_price ?? 0).toFixed(2).replace(".", ",")} / mês</strong></article>)}</div> : <EmptyState title="Nenhum plano vinculado" description="O plano contratado aparecerá aqui." />}</section></>;
    if (section === "contrato") return <><PageHeader {...copy} /><section className="panel">{contracts?.length ? <div className="form-stack">{contracts.map((contract: any) => <article className="mission-card" key={contract.id}><div className="flex space-between gap-8 wrap"><strong>Contrato Plumareli</strong><Badge tone={contract.status === "signed" ? "green" : "yellow"}>{contract.status}</Badge></div><small className="muted">{contract.signed_at ? `Assinado em ${dt(contract.signed_at)}` : `Criado em ${dt(contract.created_at)}`}</small></article>)}</div> : <EmptyState title="Nenhum contrato disponível" description="Quando o contrato for emitido, ele aparecerá aqui." />}</section></>;
