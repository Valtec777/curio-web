...
export default async function FamilyProgressPage({ searchParams }: { searchParams: Promise<{ aluno?: string }> }) {
  ...
  const scores = [...completedMissionRows.map((item) => Number(item.after_score)).filter(Number.isFinite), ...reviewedNotebookRows.map((item) => Number(item.score)).filter(Number.isFinite), ...assessmentRows.map((item) => Number(item.score)).filter(Number.isFinite)];
  ...
  const reinforce = stateRows.filter((state) => Number(state.evidence_count) >= 2 && Number(state.domain_level) < 3).sort((a, b) => Number(b.priority || 0) - Number(a.priority || 0));
  const improving = stateRows.filter((state) => state.trend === "improving").length;

  const subjectScores = new Map<string, number[]>();
  const addSubjectScore = (subject: string | undefined, score: unknown) => { const value = Number(score); if (!subject || !Number.isFinite(value)) return; const values = subjectScores.get(subject) || []; values.push(value); subjectScores.set(subject, values); };
