...
export default async function FamilyAssessmentsPage({
  searchParams,
}: {
  ...
  if (!selectedChild) {
    return <EmptyState title="Nenhuma criança vinculada" description="As avaliações aparecerão depois que houver uma criança vinculada." />;
  }
  ...
  const [{ data: subjects }, { data: assigned }, { data: reported }] = await Promise.all([
    ...
    supabase
      ...
      .select("id,status,score,teacher_note,submitted_at,reviewed_at,assessments(title,scheduled_for,instructions,file_path,subjects(name))")
      .eq("student_id", selectedChild.student_id)
      .order("created_at", { ascending: false })
      .limit(60),
    supabase
      .from("family_assessment_reports")
      .select("id,origin,title,assessment_date,content,observations,file_path,file_name,status,created_at,subjects(name)")
      .eq("student_id", selectedChild.student_id)
    ...
    <>
      ...
      <section className="panel family-highlight">
        ...
        <form action={reportFamilyAssessment} className="form-stack">
          ...
          <div className="form-row">
            <div className="field"><label>Origem</label><select className="select" name="origin" defaultValue="school"><option value="school">Escola</option><option value="guardian">Responsável</option></select></div>
            <div className="field"><label>Data *</label><input className="input" type="date" name="assessmentDate" required /></div>
          </div>
      ...
      <div className="grid-2">
              ...
              {assignedRows.map((item: any) => (
                <article className="family-upload-card" key={item.id}>
                  <div className="flex space-between gap-8 wrap">
                    <div>
                      <Badge tone="blue">{item.assessments?.subjects?.name || "Avaliação"}</Badge>
                      <h3>{item.assessments?.title || "Avaliação"}</h3>
                      <p>{item.assessments?.instructions || "Sem observações adicionais."}</p>
                    </div>
                    {item.score != null ? <Badge tone="green">Nota {item.score}/100</Badge> : <Badge tone="yellow">{item.status === "assigned" ? "Agendada" : item.status}</Badge>}
                  </div>
                  <small className="muted">Data: {date(item.assessments?.scheduled_for)}</small>
                  {item.teacher_note ? <p className="mt-12"><strong>Devolutiva da professora:</strong> {item.teacher_note}</p> : null}
                  {assignedFiles.get(item.id) ? <div className="mt-12"><a className="button button-secondary button-small" href={assignedFiles.get(item.id)} target="_blank" rel="noreferrer">Abrir arquivo ↗</a></div> : null}
                </article>
              ))}
            </div>
        ...
        <section className="panel">
          <div className="panel-head"><div><h2>Informadas pela família/escola</h2><p>Histórico do que foi enviado para a equipe.</p></div></div>
          {reportedRows.length ? (
            <div className="form-stack">
              {reportedRows.map((item: any) => (
                <article className="family-upload-card" key={item.id}>
                  <div className="flex space-between gap-8 wrap">
                    <div><Badge tone="purple">{item.origin === "school" ? "Escola" : "Responsável"}</Badge><h3>{item.title}</h3><p>{item.subjects?.name || "Sem matéria"} · {date(item.assessment_date)}</p></div>
                    <Badge tone={item.status === "linked" ? "green" : "neutral"}>{item.status === "linked" ? "Usada no acompanhamento" : "Informada"}</Badge>
                  </div>
                  {item.content ? <p><strong>Conteúdo:</strong> {item.content}</p> : null}
                  {item.observations ? <p>{item.observations}</p> : null}
                  {reportedFiles.get(item.id) ? <a className="button button-secondary button-small" href={reportedFiles.get(item.id)} target="_blank" rel="noreferrer">Abrir anexo ↗</a> : null}
                </article>
              ))}
            </div>
          ) : <EmptyState title="Sem avaliações informadas" description="Use o formulário acima quando receber uma prova ou avaliação da escola." />}
        </section>
      </div>
    </>
  );
