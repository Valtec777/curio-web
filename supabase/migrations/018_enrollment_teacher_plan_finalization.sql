-- CURIÓ · Finalização operacional da matrícula
-- Reaproveita access_invitations como registro de entrada e subscriptions/teacher_students existentes.

alter table public.access_invitations
  add column if not exists teacher_id uuid references public.teachers(id) on delete set null,
  add column if not exists plan_id uuid references public.plans(id) on delete set null,
  add column if not exists enrollment_finalized_at timestamptz;

-- Um aluno não deve ter duas assinaturas correntes simultâneas.
create unique index if not exists subscriptions_one_current_per_student_uidx
